...
void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("thumbnormalalpha")) == 0 ) {
		DWORD dwThumbNormalAlpha = _ttoi(pstrValue);
		SetThumbNormalAlpha(dwThumbNormalAlpha);
	}
	else CControlUI::SetAttribute(pstrName, pstrValue);
}


void CScrollBarUI::SetThumbNormalAlpha(BYTE dwThumbNormalAlpha)
{
	if( m_dwThumbNormalAlpha == dwThumbNormalAlpha ) return;
	m_dwThumbNormalAlpha = dwThumbNormalAlpha;
	Invalidate();
}


DWORD CScrollBarUI::GetThumbNormalAlpha() const
{
	return m_dwThumbNormalAlpha;
}
...
void CScrollBarUI::PaintThumb(HDC hDC)
{
	...
	if(m_dwThumbNormalAlpha > 0)
	{
		m_diThumbNormal.uFade = m_dwThumbNormalAlpha;
	}
