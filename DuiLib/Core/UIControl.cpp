...
m_nTooltipWidth(300),
m_bVGradient(false),
m_dwGradient2Percent(50),
m_dwMaskStartColor(0),
m_dwMaskEndColor(0),
...
DWORD CControlUI::GetFocusedBkColor() const
{
	return m_dwFocusedBackColor;
}

void CControlUI::SetFocusedBkColor(DWORD dwBackColor)
{
...
bool CControlUI::GetVGradient() const
{
	return m_bVGradient;
}

void CControlUI::SetVGradient(bool bVGradient)
{
	if( m_bVGradient == bVGradient ) return;

	m_bVGradient = bVGradient;
	Invalidate();
}

DWORD CControlUI::GetGradient2Percent() const
{
	return m_dwGradient2Percent;
}

void CControlUI::SetGradient2Percent(DWORD dwGradient2Percent)
{
	if( m_dwGradient2Percent == dwGradient2Percent ) return;

	m_dwGradient2Percent = dwGradient2Percent;
	Invalidate();
}

DWORD CControlUI::GetMaskStartColor() const
{
	return m_dwMaskStartColor;
}

void CControlUI::SetMaskStartColor(DWORD dwMaskStartColor)
{
	if(m_dwMaskStartColor == dwMaskStartColor ) return;

	m_dwMaskStartColor = dwMaskStartColor;
	Invalidate();
}

DWORD CControlUI::GetMaskEndColor() const
{
	return m_dwMaskEndColor;
}

void CControlUI::SetMaskEndColor(DWORD dwMaskEndColor)
{
	if(m_dwMaskEndColor == dwMaskEndColor ) return;

	m_dwMaskEndColor = dwMaskEndColor;
	Invalidate();
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("focusedbkcolor")) == 0 || _tcscmp(pstrName, _T("bkcolor1")) == 0 ) {
	...
	else if( _tcscmp(pstrName, _T("vgradient")) == 0 ) {
		SetVGradient(_tcscmp(pstrValue, _T("true")) == 0);
	}
	else if( _tcscmp(pstrName, _T("gradient2percent")) == 0 ) {
		DWORD dwGradient2Percent = _ttoi(pstrValue);
		SetGradient2Percent(dwGradient2Percent);
	}
	else if( _tcscmp(pstrName, _T("maskstartcolor")) == 0 ) {
		while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetMaskStartColor(clrColor);
	}
	else if( _tcscmp(pstrName, _T("maskendcolor")) == 0 ) {
		while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetMaskEndColor(clrColor);
	}
...
{
	if(IsFocused() && m_dwFocusedBackColor !=0 )
	{
		CRenderEngine::DrawColor(hDC, m_rcItem, GetAdjustColor(m_dwFocusedBackColor));
	}
	...
	if(m_dwMaskStartColor != 0 && m_dwMaskEndColor != 0)
	{
		if(!GetVGradient())
		{
			RECT rc = m_rcItem;
			rc.bottom = rc.top + m_dwGradient2Percent* (rc.bottom - rc.top) / 100;
			CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwMaskStartColor), GetAdjustColor(m_dwMaskEndColor), true, 8);
		}
		else
		{
			RECT rc = m_rcItem;
			rc.right = rc.left + m_dwGradient2Percent* (rc.right - rc.left) / 100;
			CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwMaskStartColor), GetAdjustColor(m_dwMaskEndColor), false, 8);
		}
	}
}
...
{
	if((m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)) {//m_rcBorderSize.left > 0 && 
