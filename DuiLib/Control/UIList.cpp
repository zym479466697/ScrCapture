...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
    {
        if( IsEnabled() ) {
			...
			if(event.Type == UIEVENT_BUTTONDOWN)
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK_FINISHED);
			}
        }
        return;
    }
...
CListHBoxElementUI::CListHBoxElementUI():m_pCallBack(NULL)
{
...
void CListHBoxElementUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (pInfo->nColumns > 0) {
		...
		if(m_pCallBack) m_pCallBack->OnSizeChange(this);
    }
    else {
        for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
            CControlUI* pControl = static_cast<CControlUI*>(m_items[it2]);
...
void CListHBoxElementUI::SetCallBackObj(ImpPosChangeCallBack* pCallBack)
{
	m_pCallBack = pCallBack;
}
