...
void CPaintManagerUI::SetFirstFocusNeeded(bool bNeed)
{
	m_bFocusNeeded = bNeed;
}


bool CPaintManagerUI::AttachDialog(CControlUI* pControl, bool bAutoDeleteControl/*=true*/)
{
...
HFONT CPaintManagerUI::GetFont(int id)
{
	...
	if (!pFontInfo) 
	{
		if( m_pParentResourcePM ) 
			return m_pParentResourcePM->GetFont(id);
...
CDuiString CPaintManagerUI::GetOmitStr(LPCTSTR pStr, int nSize, int fontId, int limitWidth)
{
	SIZE size = {0}, omitSize = {0};
	LPCTSTR pStrOmit = _T("...");
	TFontInfo* pFontInfo = GetFontInfo(fontId);
	int nWidth = nSize;
	HFONT hOldFont = (HFONT) ::SelectObject(m_hDcPaint, pFontInfo->hFont);
	::GetTextExtentPoint32(m_hDcPaint, pStr, nWidth, &size);
	::GetTextExtentPoint32(m_hDcPaint, pStrOmit, 3, &omitSize);
	while(omitSize.cx + size.cx > limitWidth)
	{
		::GetTextExtentPoint32(m_hDcPaint, pStr, nWidth--, &size);
	}
	::SelectObject(m_hDcPaint, hOldFont);
