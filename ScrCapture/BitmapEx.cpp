...
void CBitmapEx::_RotateNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		long _width = (long)((float)absex((float)m_bih.biWidth*cos(_angle)) + (float)absex((float)m_bih.biHeight*sin(_angle)) + 0.5f);
		long _height = (long)((float)absex((float)m_bih.biWidth*sin(_angle)) + (float)absex((float)m_bih.biHeight*cos(_angle)) + 0.5f);
		long _pitch = m_iBpp * _width;
		while ((_pitch & 3) != 0)
			_pitch++;
...
void CBitmapEx::_RotateBilinear(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		// Calculate rotating params
		float _angle = ((float)-degrees/180.0f) * _PI;
		long _width = (long)(absex((float)m_bih.biWidth*cos(_angle)) + absex((float)m_bih.biHeight*sin(_angle)) + 0.5f);
		long _height = (long)(absex((float)m_bih.biWidth*sin(_angle)) + absex((float)m_bih.biHeight*cos(_angle)) + 0.5f);
		long _pitch = m_iBpp * _width;
		while ((_pitch & 3) != 0)
			_pitch++;
		fixed f_0_5 = ftofx(0.5f);
...
void CBitmapEx::_ShearVerticalNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		long _height = m_bih.biHeight + absex(_offset);
...
void CBitmapEx::_ShearHorizontalNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		long _width = m_bih.biWidth + absex(_offset);
		fixed f_alpha = ftofx(_alpha);
		fixed f_offset = ftofx(_offset);
...
void CBitmapEx::UpdateFireEffect(BOOL bLarge, long iteration, long height)
{
	...
	if (IsValid() && (m_lpFire != NULL))
	{
		...
		for (long k=0; k<iteration; k++)
		{
			...
			for(j=0; j<m_bih.biWidth; j++)
			{
				m_lpFire[dwOffset] = (int)abs(32768 + rand()) % 256;
				dwOffset++;
			}
