    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
			...
			case 11:
				...
				else if (_tcsicmp(pstrClass, DUI_CTR_MACPROGRESS) == 0)		  pControl = new CMacProgressUI;
				break;
			case 13:
				if (_tcsicmp(pstrClass, DUI_CTR_DYNAMICLAYOUT) == 0)			  pControl = new CDynamicLayoutUI;
				break;
            case 14:
