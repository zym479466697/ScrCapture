...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
	...
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bDrawCaret(true), m_bInited(false), m_dwHotBorderColor(0), m_uButtonState(0), 
	m_hCurIMC(NULL), m_enableIme(true), m_bNumberOnly(false)
{
	::ZeroMemory(&m_rcTextPadding, sizeof(m_rcTextPadding));
	::ZeroMemory(&m_rcPlaceholderPadding, sizeof(m_rcPlaceholderPadding));
}
...
DWORD CRichEditUI::GetPlaceholderTextColor()
{
	return m_dwPlaceholderTextColor;
}

void CRichEditUI::SetPlaceholderTextColor(DWORD dwPlaceholderTextColor)
{
	m_dwPlaceholderTextColor = dwPlaceholderTextColor;
	if( m_pTwh ) {
		m_pTwh->SetColor(m_dwPlaceholderTextColor);
	}
}

DWORD CRichEditUI::GetHotBorderColor() const
{
	return m_dwHotBorderColor;
}

void CRichEditUI::SetHotBorderColor(DWORD dwHotBorderColor)
{
...
void CRichEditUI::SetPlaceholderText(LPCTSTR pstrPlaceholderText)
{
	if(_tcscmp(m_sPlaceholderText, pstrPlaceholderText) == 0) return;
	m_sPlaceholderText = pstrPlaceholderText;
	Invalidate();
}
...
bool CRichEditUI::CanUndo()
{
	...
	m_pTwh->GetTextServices()->TxSendMessage(EM_CANUNDO, 0, 0, &lResult);
	return lResult;
}

bool CRichEditUI::GetSel(DWORD& dwBegin, DWORD& dwEnd)
{
	if( !m_pTwh ) return false;
	m_pTwh->GetTextServices()->TxSendMessage(EM_GETSEL, (WPARAM)&dwBegin, (LPARAM)&dwEnd, 0);
	return true;
}
...
RECT CRichEditUI::GetPlaceholderPadding() const
{
	return m_rcPlaceholderPadding;
}

void CRichEditUI::SetPlaceholderPadding(RECT rc)
{
	m_rcPlaceholderPadding = rc;
	Invalidate();
}
...
void CRichEditUI::DoEvent(TEventUI& event)
{
	...
	if (event.Type == UIEVENT_SETCURSOR && IsEnabled())
	{
		...
		if (m_pTwh) {
			if (!m_enableIme)
			{
				m_hCurIMC = ImmAssociateContext(m_pManager->GetPaintWindow(), NULL);
			}
			m_pTwh->OnTxInPlaceActivate(NULL);
	...
	if (event.Type == UIEVENT_KILLFOCUS) {
		if (m_pTwh) {
			if (!m_enableIme)
			{
				ImmAssociateContext(m_pManager->GetPaintWindow(), m_hCurIMC);
				m_hCurIMC = NULL;
			}
			m_pTwh->OnTxInPlaceActivate(NULL);
...
//添加鼠标进入后变色
void CRichEditUI::PaintBorder(HDC hDC)
{
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    if( _tcscmp(pstrName, _T("autohscroll")) == 0 ) {
		...
		if (_tcscmp(pstrValue, _T("true")) == 0) m_bNumberOnly = true;
	}
	else if (_tcscmp(pstrName, _T("enableIme")) == 0) {
		m_enableIme = _tcscmp(pstrValue, _T("true")) == 0;
	}
	...
	else if (_tcscmp(pstrName, _T("placeholderpadding")) == 0) {
		RECT rcPlaceholderPadding = { 0 };
		LPTSTR pstr = NULL;
		rcPlaceholderPadding.left = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);
		rcPlaceholderPadding.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);
		rcPlaceholderPadding.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);
		rcPlaceholderPadding.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
		SetPlaceholderPadding(rcPlaceholderPadding);
	}
	else if (_tcscmp(pstrName, _T("limit")) == 0)
	{
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    ...
    if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
		...
		CreateMenu();
		GetManager()->SendNotify((CControlUI*)this, DUI_MSGTYPE_MENU);
		bHandled = TRUE;
		return 0;
    }
    else
...
void CRichEditUI::CreateMenu()
{
	...
	if(m_pTwh)
	{
		//HWND hwnd = pHover->GetManager()
		//::SetForegroundWindow(hWnd); 
		DWORD dwBegin, dwEnd;
		m_pTwh->GetTextServices()->TxSendMessage(EM_GETSEL, (WPARAM)&dwBegin, (LPARAM)&dwEnd, 0);  
		BOOL bCanUndo = m_pTwh->GetTextServices()->TxSendMessage(EM_CANUNDO, 0, 0, 0);  
		...
		switch(cmd)
		{
		case MENU_UNDO: this->Undo();break;
		case MENU_CUT: this->Cut();break;
		case MENU_COPY: this->Copy();break;
		case MENU_PASTE: this->Paste();break;
		default:
			{
...
void CRichEditUI::PaintText(HDC hDC)
{
	...
	RECT rc = m_rcItem;
	rc.left += m_rcPlaceholderPadding.left;
	rc.right -= m_rcPlaceholderPadding.right;
	rc.top += m_rcPlaceholderPadding.top;
	rc.bottom -= m_rcPlaceholderPadding.bottom;
